...
function TaskItem({ taskItem, onUpdate, onDelete }) {
  ...
  // Highlighting overdue tasks for visibility
  const now = new Date();
  ...
  // Enhanced styling for urgent tasks (within 12 hours)
  ...
  return (
    <>
      <div
        className={`p-4 mb-2 rounded-lg ${isDueWithin12Hours ? 'shadow-md' : 'shadow-sm'} bg-white relative ${overdueStyle} hover:bg-gray-50 transition-colors ${isDueWithin12Hours ? 'animate-pulse-slow' : ''}`}
      ...
      >
        ...
        <div onClick={() => setShowDetails(true)} className="cursor-pointer pr-20">
          <div className="flex items-center mb-1">
            <h3 className="font-semibold flex-1">{taskItem.taskHeading}</h3>
